  ...
  <div class="wrapper register" id="registers">
    <el-dialog title="退费详情" :visible.sync="isShow.show" width="960px" @open="openGetData">
        ...
        <el-row>
          <el-col :span="11">
            <el-form-item label="交易时间">
              <span>{{getTime(otherData.sfTime)}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item label="申请退款时间" label-width="120px">
              <span>{{getTime(otherData.tfTime)}}</span>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="退款金额">
              <span class="red">￥{{otherData.refundMoney}}</span>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="备注">
              <span>{{otherData.approvalRemark}}</span>
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <ul class="btns_wrap">
          <li class="actives item" v-if="otherData.billingStatus == 6">
            <span @click="handleCkickOne(7)">审批通过</span>
          </li>
          <li class="item" v-if="otherData.billingStatus == 6">
            <span @click="handleCkickOne(8)">驳回</span>
          </li>
...
<script>
import { getDetailOne, putDrugs } from "@/api";
...
export default {
  ...
  methods: {
    ...
    openGetData() {
      getDetailOne(this.dataId).then(res => {
        console.log( res )
        if (res.data.code == 200) {
          let arr = ["待处理", "已通过", "已驳回"];
          this.tableData = res.data.data.refundSlipItemList;
          this.otherData = res.data.data.chargeBilling;
          this.otherData.statusTxt = arr[parseInt(this.otherData.billingStatus) - 6]; // 678
          this.otherData.usernames = this.otherData.createUser.name;
        }
      });
    }
  },
